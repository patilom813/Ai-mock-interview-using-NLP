  ...
  const GetFeedback = async () => {
    try {
      const result = await db
        .select()
        .from(UserAnswer) // Use lowercase `userAnswer`
        .where(and(eq(UserAnswer.mockIdRef, 1), eq(UserAnswer.user_id, userId))) // Combine conditions
      ...
      {feedbackList?.length == 0 ? (
        ...
        <>
          ...
          <h2 className="text-primary text-lg my-3">
            ...
            <strong
              className={`${overallRating >= 4 ? "text-green-500" : "text-red-600"
                }`}
            >
              {overallRating}
