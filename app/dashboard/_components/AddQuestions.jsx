...
const AddQuestions = () => {
  ...
  const onSubmit = async (e) => {
    ...
    Previous Questions from Company: ${company},
    Based on this information, please provide 5 interview questions with answers in JSON format.
    Ensure each object has "Question" and "Answer" as fields.
  `;
    ...
    try {
      const result = await chatSession.sendMessage(InputPrompt);
      let MockQuestionJsonResp = await result.response.text();

      // Remove markdown code block formatting and extra spaces
      MockQuestionJsonResp = MockQuestionJsonResp.replace(/```(json)?/g, "").trim();
      MockQuestionJsonResp = MockQuestionJsonResp.replace(/```?/g, "").trim();
      console.log("Cleaned JSON RESPONSE:", MockQuestionJsonResp);

      // Attempt to parse JSON to verify correctness
      try {
        const parsedData = JSON.parse(MockQuestionJsonResp);
        console.log("Parsed Data:", parsedData);
      } catch (jsonError) {
        console.error("Invalid JSON Format:", jsonError.message);
        alert("Received invalid JSON format. Please try again.");
        return;
      }
